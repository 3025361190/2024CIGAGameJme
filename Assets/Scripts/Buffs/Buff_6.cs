...
public class Buff_6 : BaseBuff
{
    private float value;
    private float originalSpeedAfterSplit;
    ...
    public override void Init()
    {
        ...
        originalSpeedAfterSplit = GameManager.Instance.data.bulletData["speedAfterSplit"].ToObject<float>();
        // 初始化运行时参数
        currentStack = 0;
        isActive = false;
        target = null;
    ...
    public override void ActivateBuff()
    {
        ...
        if(stackType == 0)
        {
            GameManager.Instance.data.bulletData["speedAfterSplit"] = GameManager.Instance.data.bulletData["speedAfterSplit"].ToObject<float>() + value;
        }
        else if(stackType == 1)
        {
            GameManager.Instance.data.bulletData["speedAfterSplit"] = GameManager.Instance.data.bulletData["speedAfterSplit"].ToObject<float>() * value;
    ...
    {
        base.DeactivateBuff();
        GameManager.Instance.data.bulletData["speedAfterSplit"] = originalSpeedAfterSplit;
    }    
}
