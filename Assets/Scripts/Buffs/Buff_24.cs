/*
文件名：Buff_24.cs
编辑人：没道理啊
文件描述：稀有buff4
*/
using System.Collections;
using System.Collections.Generic;
using UnityEngine;
using System.Linq;
using Newtonsoft.Json.Linq;

public class Buff_24 : BaseBuff
{
    private float value;
    private float originalsplitNum;
    private float originalsplitModeDuration;

    public override void Init()
    {
        // 初始化buff参数
        buffId = 24;
        // 在buffs数组中查找对应buffId的配置
        var buffsArray = GameManager.Instance.data.buffData["buffs"] as JArray;
        var buffConfig = buffsArray.FirstOrDefault(b => b["buffId"].ToObject<int>() == buffId);
        buffName = buffConfig["buffName"].ToString();
        buffDescription = buffConfig["buffDescription"].ToString();
        buffIcon = buffConfig["buffIcon"].ToString();
        buffDuration = buffConfig["buffDuration"].ToObject<float>();
        buffStackable = buffConfig["buffStackable"].ToObject<bool>();
        stackType = buffConfig["stackType"].ToObject<int>();
        isNormalBuff = buffConfig["isNormalBuff"].ToObject<bool>();
        value = buffConfig["value"].ToObject<float>();
        originalsplitNum = GameManager.Instance.data.bulletData["splitNum"].ToObject<float>();
        originalsplitModeDuration = GameManager.Instance.data.modeData["splitModeDuration"].ToObject<float>();
        // 初始化运行时参数
        currentStack = 0;
        isActive = false;
        target = null;
    }

    public override void ActivateBuff()
    {
        // 保留父类逻辑
        base.ActivateBuff();
        
        GameManager.Instance.data.bulletData["splitNum"] = GameManager.Instance.data.bulletData["splitNum"].ToObject<float>() * 5;
        GameManager.Instance.data.modeData["splitModeDuration"] = 5.0f;
    }

    public override void DeactivateBuff()
    {
        base.DeactivateBuff();
        GameManager.Instance.data.bulletData["splitNum"] = originalsplitNum;
        GameManager.Instance.data.modeData["splitModeDuration"] = originalsplitModeDuration;
    }    
}
